...
const globalContextInfo = {
    ...
    forwardedNewsletterMessageInfo: {
        newsletterJid: process.env.CHANNEL_JID || '120363200367779016@newsletter',
        newsletterName: process.env.CHANNEL_NAME || 'MUMU MINI BOT',
        serverMessageId: 144
    }
};
...
async function connectToWhatsApp() {
    ...
    if (config.USE_PAIR_CODE && !state.creds.registered) {
        setTimeout(async () => {
            try {
                const phone = (config.OWNER_NUMBER || '').replace(/\D/g, '');
                if (!phone) {
                    logMessage('WARN', '⚠️ OWNER_NUMBER not set — cannot request pair code');
                    return;
                }
                logMessage('INFO', `📲 Requesting pair code for +${phone}...`);
                const code = await sock.requestPairingCode(phone);
                global.pairCode = code;
                global.pairCodePhone = phone;
                logMessage('SUCCESS', `\n\n╔══════════════════════════╗\n║  PAIR CODE: ${code.padEnd(14)}║\n╚══════════════════════════╝\n\nEnter this code in WhatsApp > Linked Devices > Link with phone number\n`);
            } catch (e) {
                logMessage('WARN', `Pair code request failed: ${e.message}`);
            }
        }, 3000);
    }
            ...
            setTimeout(async () => {
                ...
                try {
                    ...
                    if (channelLink.includes('whatsapp.com/channel/')) {
                        ...
                        if (match) {
                            ...
                            if (meta?.id) {
                                ...
                                globalContextInfo.forwardedNewsletterMessageInfo.newsletterJid = meta.id;
                                if (!global._followedNewsletters.has(meta.id)) {
                                    await sock.newsletterFollow(meta.id);
                                    global._followedNewsletters.add(meta.id);
                                    logMessage('INFO', `✅ Startup: following channel ${meta.id}`);
                                }
                            }
                        }
                    }
                    ...
                    if (/already|409|subscribed|unexpected response/i.test(msg)) {
                        logMessage('INFO', `✅ Already following channel`);
                    } else {
                        logMessage('WARN', `Channel follow failed: ${msg}`);
                    }
                }
            }, 5000);
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        try {
            ...
            for (const m of messages) {
                ...
                // Process notify (incoming from others) AND fromMe append (owner's own commands
                // from their phone — needed when the bot runs on the owner's own WhatsApp number)
                const isFromMe = m.key?.fromMe === true;
                if (type && type !== 'notify' && !(isFromMe && type === 'append')) continue;
...
// ✅ Pair code endpoint — lets the web dashboard display the code
app.get('/pair-code', (req, res) => {
    res.json({
        code: global.pairCode || null,
        phone: global.pairCodePhone || null,
        connected: !!global.botJid
    });
});

// ✅ Session ID generator — exports current session as Silva~<base64> string
app.get('/session', (req, res) => {
    try {
        const credsFile = path.join(__dirname, 'session', 'creds.json');
        if (!fs.existsSync(credsFile)) {
            return res.status(404).json({ error: 'No session found. Pair the bot first.' });
        }
        const raw = fs.readFileSync(credsFile);
        const compressed = zlib.gzipSync(raw);
        const b64 = compressed.toString('base64');
        const sessionId = `Silva~${b64}`;
        res.json({ session_id: sessionId, note: 'Copy this value and set it as SESSION_ID on Render.' });
    } catch (e) {
...
(async () => {
    try {
        console.log('\x1b[36m');
        console.log('╔══════════════════════════════════════════╗');
        console.log('║       MUMU MINI BOT — WhatsApp Bot       ║');
        console.log('║                                           ║');
        console.log('║   Owner: 263776719736                     ║');
        console.log('║   Channel: whatsapp.com/channel/...       ║');
        console.log('║   Node.js  •  Baileys  •  v2.0            ║');
        console.log('╚══════════════════════════════════════════╝');
        console.log('\x1b[0m');
        logMessage('INFO', 'Booting MUMU MINI BOT...');
